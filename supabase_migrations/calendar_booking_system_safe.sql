-- Safe Calendar Booking System Migration
-- This migration safely adds calendar booking functionality, handling existing objects

-- Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can view their own booking links" ON public.booking_links;
DROP POLICY IF EXISTS "Users can insert their own booking links" ON public.booking_links;
...
DROP POLICY IF EXISTS "Service roles can insert bookings" ON public.bookings;
DROP POLICY IF EXISTS "Users can update their own bookings" ON public.bookings;
DROP POLICY IF EXISTS "Users can manage their booking availability" ON public.booking_availability;

-- Drop existing triggers first (they depend on functions)
DROP TRIGGER IF EXISTS trigger_update_lead_on_booking ON bookings;

-- Drop existing functions if they exist (use CASCADE to handle any remaining dependencies)
DROP FUNCTION IF EXISTS check_booking_availability(BIGINT, TIMESTAMPTZ, TIMESTAMPTZ) CASCADE;
DROP FUNCTION IF EXISTS update_lead_on_booking() CASCADE;
DROP FUNCTION IF EXISTS get_available_slots(BIGINT, DATE, TEXT) CASCADE;
...
-- Add booking-related columns to leads table (safely)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE leads ADD COLUMN booking_id BIGINT;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'leads_booking_id_fkey'
    ) THEN
        ALTER TABLE leads ADD CONSTRAINT leads_booking_id_fkey 
...
DROP INDEX IF EXISTS idx_booking_links_active;
DROP INDEX IF EXISTS idx_bookings_user_id;
DROP INDEX IF EXISTS idx_bookings_lead_id;
DROP INDEX IF EXISTS idx_bookings_start_time;
DROP INDEX IF EXISTS idx_bookings_status;
DROP INDEX IF EXISTS idx_bookings_nylas_event_id;
DROP INDEX IF EXISTS idx_booking_availability_link_date;
DROP INDEX IF EXISTS idx_leads_call_scheduled;
DROP INDEX IF EXISTS idx_leads_call_scheduled_date;

CREATE INDEX idx_booking_links_user_id ON booking_links(user_id);
CREATE INDEX idx_booking_links_slug ON booking_links(booking_slug);
CREATE INDEX idx_booking_links_active ON booking_links(is_active) WHERE is_active = TRUE;

CREATE INDEX idx_bookings_user_id ON bookings(user_id);
CREATE INDEX idx_bookings_lead_id ON bookings(lead_id);
CREATE INDEX idx_bookings_start_time ON bookings(start_time);
CREATE INDEX idx_bookings_status ON bookings(status);
CREATE INDEX idx_bookings_nylas_event_id ON bookings(nylas_event_id);

CREATE INDEX idx_booking_availability_link_date ON booking_availability(booking_link_id, date);

CREATE INDEX idx_leads_call_scheduled ON leads(call_scheduled) WHERE call_scheduled = TRUE;
CREATE INDEX idx_leads_call_scheduled_date ON leads(call_scheduled_date);
...
DO $$
BEGIN
    RAISE NOTICE 'Calendar Booking System migration completed successfully!';
    RAISE NOTICE 'Tables created: booking_links, bookings, booking_availability';
    RAISE NOTICE 'Functions created: check_booking_availability, get_available_slots, update_lead_on_booking';
