  ...
  const fetchEmailThreads = async () => {
    try {
      ...
      const supabase = createBrowserClient(
        ...
        process.env.NEXT_PUBLIC_SUPABASE_SERVICE_KEY! // Use service role key
      );

      const specificUserId = 'f7ee9f97-dead-4f92-99de-23cd707e3f0c';
      
      console.log('Querying enhanced replies with service role key...');
      ...
      if (repliesData && repliesData.length > 0) {
        ...
        const transformedReplies = repliesData.map(reply => ({
          ...
          lead_name: `${reply.leads?.first_name || ''} ${reply.leads?.last_name || ''}`.trim() || reply.sender_name || 'Unknown Lead',
          lead_email: reply.sender_email || 'unknown@email.com',
          lead_company: reply.leads?.company || 'Unknown Company',
          thread_subject: reply.subject || 'No Subject',
          lead_reply: {
            content: reply.body || 'No message content',
          ...
          last_activity: reply.received_at || reply.created_at
        }));
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{inboxStats.totalConversations}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{inboxStats.autoReplied}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{inboxStats.needsAttention}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{inboxStats.responseRate}%</p>
              </div>
